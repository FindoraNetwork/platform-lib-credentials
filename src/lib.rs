...
use {
    ...
    platform_lib_noah::noah_api::{
        anon_creds::{
            ...
            ACCommitmentKey, ACIssuerPublicKey, ACIssuerSecretKey, ACPoK, ACRevealProof,
            ACRevealSig, ACSignature, ACUserPublicKey, ACUserSecretKey,
            Credential as NoahCredential,
        },
        errors::NoahError,
    },
...
pub fn credential_open_commitment<R: CryptoRng + RngCore>(
    ...
    let reveal_map = reveal_field_to_bitmap(&credential.issuer_pub_key, reveal_fields).c(d!())?;
    let ac_credential = credential.to_ac_credential().c(d!())?;
